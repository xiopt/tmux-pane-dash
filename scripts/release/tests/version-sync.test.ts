...
test("reports shared contract VERSION and TAG mismatches", async () => {
  ...
  await expect(inspectVersions(root)).resolves.toMatchObject({
    ...
    tag: "v0.1.3",
    mismatches: [
      "scripts/release/contracts.ts: VERSION 0.1.2 !== VERSION 0.1.3",
      "scripts/release/contracts.ts: TAG v0.1.2 !== v0.1.3",
    ],
  })
})
...
test("requires root identity files even when VERSION exists", async () => {
  ...
  await expect(inspectVersions(root)).resolves.toMatchObject({
    mismatches: [
      "scripts/release/contracts.ts: VERSION 0.1.2 !== VERSION 0.1.0",
      "scripts/release/contracts.ts: TAG v0.1.2 !== v0.1.0",
...
test("accepts historical packed tags, the loose current tag, duplicates, and non-v tags", async () => {
  const root = await synchronizedFixture("0.1.2")
  await mkdir(join(root, ".git", "refs", "tags"), { recursive: true })
  await writeFile(join(root, ".git", "refs", "tags", "v0.1.2"), "current\n")
  ...
  await expect(inspectVersions(root)).resolves.toEqual({ version: "0.1.2", tag: "v0.1.2", mismatches: [] })
})
...
test("rejects malformed and future v tags while retaining historical parity", async () => {
  ...
  await expect(inspectVersions(root)).resolves.toMatchObject({
    ...
    mismatches: [
      ...
      "tag v0.1.3: future tag is newer than VERSION 0.1.2",
    ],
  })
})
