...
function defaultStdout(argv: readonly string[]): string {
  ...
  if (command.includes("--version")) return "pane-dash 0.1.7\n"
...
test.skipIf(process.platform !== "darwin")("observes the exact scoped OpenCode plugin spec with npm-package-arg", async () => {
  await expect(observeOpenCodePluginSpec("@xiopt/pane-dash-opencode@0.1.7")).resolves.toEqual({
    name: "@xiopt/pane-dash-opencode",
    rawSpec: "0.1.7",
  })
  for (const invalid of ["pane-dash-opencode@0.1.7", "@xiopt/pane-dash-opencode", "@xiopt/pane-dash-opencode@v0.1.7", "@xiopt/pane-dash-opencode@0.1"]) {
...
test("refuses an unvalidated ambient npm-package-arg root", async () => {
  ...
  try {
    await expect(observeOpenCodePluginSpec("@xiopt/pane-dash-opencode@0.1.7")).rejects.toThrow("PANE_DASH_NPA_ROOT")
  } finally {
...
test("fails closed when the Seatbelt parser returns malformed or noncanonical package observations", async () => {
  ...
  try {
    ...
    await expect(observeOpenCodePluginSpec("@xiopt/pane-dash-opencode@0.1.7")).rejects.toThrow()
    for (const { version, parsed } of [
      { version: "13.0.1", parsed: { name: "@xiopt/pane-dash-opencode", rawSpec: "0.1.7" } },
      { version: "13.0.2", parsed: { name: "wrong", rawSpec: "0.1.7" } },
...
test.skipIf(process.platform !== "darwin")("observeOpenCodePluginSpec launches parser under /usr/bin/sandbox-exec with PANE_DASH_NPA_ROOT profile", async () => {
  ...
  try {
    ...
    const result = await observeOpenCodePluginSpec("@xiopt/pane-dash-opencode@0.1.7")
    expect(result).toEqual({ name: "@xiopt/pane-dash-opencode", rawSpec: "0.1.7" })
...
test("accepts only the local companion and scoped-plugin registry inventory", () => {
  ...
  expect(() => assertOpenCodeRegistryRequests([
    ...
    "/%40xiopt%2Fpane-dash-opencode/-/pane-dash-opencode-0.1.7.tgz",
  ], origin)).not.toThrow()
...
test("refuses to summarize unasserted or mutated isolation observations", () => {
  ...
  const summary = formatOpenCodePassSummary({ version: "1.17.20", sha256: "b".repeat(64), name: "@xiopt/pane-dash-opencode", rawSpec: "0.1.1", requests: [], isolation })
  ...
  for (const key of ["platform", "policySha256", "nonLoopbackConnect", "nonLoopbackConnectErrno", "loopbackRegistryConnect", "allowedSyntheticWrite", "forbiddenWrite", "publicNetworkRequests", "realHomeWrites", "defaultTmuxUses"] as const) {
    ...
    expect(() => formatOpenCodePassSummary({ version: "1.17.20", sha256: "b".repeat(64), name: "@xiopt/pane-dash-opencode", rawSpec: "0.1.1", requests: [], isolation: mutated as typeof isolation })).toThrow()
  }
})
...
test("rejects a runtime platform, binary version, tmux version, or bounded smoke timeout", async () => {
  ...
  const version = runner({ "/work/x86_64-unknown-linux-musl/release/pane-dash --version": "pane-dash 0.1.0\n" })
