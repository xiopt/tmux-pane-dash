...
test.skipIf(process.platform !== "darwin")("observes the exact scoped OpenCode plugin spec with npm-package-arg", async () => {
  await expect(observeOpenCodePluginSpec("@xiopt/pane-dash-opencode@0.1.3")).resolves.toEqual({
    name: "@xiopt/pane-dash-opencode",
    rawSpec: "0.1.3",
  })
  for (const invalid of ["pane-dash-opencode@0.1.3", "@xiopt/pane-dash-opencode", "@xiopt/pane-dash-opencode@v0.1.3", "@xiopt/pane-dash-opencode@0.1"]) {
...
test("refuses an unvalidated ambient npm-package-arg root", async () => {
  ...
  try {
    await expect(observeOpenCodePluginSpec("@xiopt/pane-dash-opencode@0.1.3")).rejects.toThrow("PANE_DASH_NPA_ROOT")
  } finally {
...
test("fails closed when the Seatbelt parser returns malformed or noncanonical package observations", async () => {
  ...
  try {
    ...
    await expect(observeOpenCodePluginSpec("@xiopt/pane-dash-opencode@0.1.3")).rejects.toThrow()
    for (const { version, parsed } of [
      { version: "13.0.1", parsed: { name: "@xiopt/pane-dash-opencode", rawSpec: "0.1.3" } },
      { version: "13.0.2", parsed: { name: "wrong", rawSpec: "0.1.3" } },
...
test.skipIf(process.platform !== "darwin")("observeOpenCodePluginSpec launches parser under /usr/bin/sandbox-exec with PANE_DASH_NPA_ROOT profile", async () => {
  ...
  try {
    ...
    const result = await observeOpenCodePluginSpec("@xiopt/pane-dash-opencode@0.1.3")
    expect(result).toEqual({ name: "@xiopt/pane-dash-opencode", rawSpec: "0.1.3" })
...
test("accepts only the local companion and scoped-plugin registry inventory", () => {
  ...
  expect(() => assertOpenCodeRegistryRequests([
    ...
    "/%40xiopt%2Fpane-dash-opencode/-/pane-dash-opencode-0.1.3.tgz",
  ], origin)).not.toThrow()
