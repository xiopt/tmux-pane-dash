...
test("archive inventory and metadata are exact and reproducible", async () => {
  ...
  try {
    ...
    expect(await inspectArchive(a, 1721740800)).toEqual([
      ["bin", "directory", "0755"], ["scripts", "directory", "0755"],
...
test("Task4 archive extracts into the installer payload inventory", async () => {
  const root = await mkdtemp(join(tmpdir(), "pane-dash-producer-consumer-"))
  try {
    await mkdir(join(root, "scripts"), { recursive: true })
    for (const path of ["pane_dash.tmux", "README.md", "LICENSE", "VERSION", "scripts/open.sh", "scripts/tag.sh"]) await writeFile(join(root, path), `${path}\n`)
    const binary = join(root, "pane-dash"), output = join(root, "archive.tar.gz"), stage = join(root, "stage")
    await writeFile(binary, new Uint8Array([0xcf, 0xfa, 0xed, 0xfe, 12, 0, 0, 1]))
    await buildArchive({ target: TARGETS["darwin-arm64"].rustTarget, binary, output, epoch: 1721740800, root })
    const fs = nodeFsOps(); await fs.mkdir(stage)
