...
const deployment = { url: "https://api.github.com/repos/xiopt/tmux-pane-dash/deployments/123", id: 123, node_id: "D_kw", sha: "0123456789abcdef0123456789abcdef01234567", ref: "v0.1.6", task: "deploy", payload: {}, original_environment: "npm-production", environment: "npm-production", description: null, creator: { login: "github-actions[bot]" }, created_at: "2026-01-01T00:00:00Z", updated_at: "2026-01-01T00:00:00Z", statuses_url: "https://api.github.com/repos/xiopt/tmux-pane-dash/deployments/123/statuses", repository_url: "https://api.github.com/repos/xiopt/tmux-pane-dash", transient_environment: false, production_environment: true, performed_via_github_app: null }
...
const input = (evidenceDir: string) => ({ runId: 42, expectedSha: "0123456789abcdef0123456789abcdef01234567", environment: "npm-production" as const, evidenceDir, comment: "approve v0.1.6" })
...
test("approval re-captures, hashes canonical request, makes one exact POST, and removes raw request", async () => {
  ...
  expect(result).toMatchObject({ response: { httpStatus: 200, runId: 42, environmentId: 99, deploymentId: 123, environment: "npm-production", sha: input(fixture.evidenceDir).expectedSha, ref: "refs/tags/v0.1.6", approved: true } })
