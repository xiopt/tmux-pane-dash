...
test("the source manifest names the approved roots and executable policy", async () => {
  ...
  expect(SOURCE_EXECUTABLES).toContain("tests/pane_dash_pty.sh")
  expect(SOURCE_EXECUTABLES).toContain("tests/pane_dash_pty_helper_test.sh")
...
test("the checked-in tree includes approved source roots and omits local docs", async () => {
  ...
  expect(entries.find((entry) => entry.path === "tests/pane_dash_pty.sh")?.mode).toBe("0755")
  expect(entries.find((entry) => entry.path === "tests/pane_dash_pty_helper_test.sh")?.mode).toBe("0755")
