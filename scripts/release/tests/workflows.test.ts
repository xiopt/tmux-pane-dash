...
const rustBuildCommands = [
  ...
  'test "$(bin/pane-dash --version)" = "pane-dash 0.1.8"',
] as const
...
test("release graph has exact least-privilege jobs, environments, handoff, and toolchains", async () => {
  ...
  expect(npm).toContain("for attempt in {1..30}; do")
  expect(npm).toContain('test "$attempt" -lt 30 || exit 1')
  expect(npm).toContain("sleep 10")
  expect(npm.match(/npm audit signatures/g)).toHaveLength(1)
  ...
  expect(text).toContain('npm publish "$RUNNER_TEMP/npm/xiopt-pane-dash-opencode-0.1.8.tgz" --access public --provenance')
...
test("draft mutation is non-clobbering and draft validation binds all six assets", async () => {
  ...
  expect(validation).not.toContain("gh release download")
  expect(validation).toContain("expected one draft release")
  expect(validation).toContain(".draft == true")
  expect(validation).toContain("releases/assets/$asset_id")
  expect(validation).toContain("test \"${#assets[@]}\" -eq 6")
...
test("npm production audits signatures without fallback bindings and verifies publication order", async () => {
  ...
  const plugin = npm.indexOf("xiopt-pane-dash-opencode-0.1.8.tgz")
  const cli = npm.indexOf("xiopt-tmux-pane-dash-0.1.8.tgz")
...
test("npm production audits only the exact published versions in an isolated no-auth project", async () => {
  const text = await workflow("release.yml")
  const npm = job(text, "npm-production")
  const install = 'npm_config_userconfig="$audit_dir/.npmrc" npm_config_cache="$RUNNER_TEMP/npm-audit-cache" npm install --prefix "$audit_dir" --ignore-scripts --no-audit --no-fund --package-lock=false @xiopt/pane-dash-opencode@0.1.8 @xiopt/tmux-pane-dash@0.1.8'
  ...
  const pluginPublish = npm.indexOf('npm publish "$RUNNER_TEMP/npm/xiopt-pane-dash-opencode-0.1.8.tgz"')
  const cliPublish = npm.indexOf('npm publish "$RUNNER_TEMP/npm/xiopt-tmux-pane-dash-0.1.8.tgz"')
...
test("draft validation verifies each exact release asset with the tagged workflow identity", async () => {
  ...
  for (const asset of [
    ...
    "tmux-pane-dash-v0.1.8-x86_64-unknown-linux-musl.tar.gz",
  ...
  expect(validation).toContain("--source-ref refs/tags/v0.1.8")
