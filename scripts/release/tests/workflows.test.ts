...
test("release graph has exact least-privilege jobs, environments, handoff, and toolchains", async () => {
  ...
  expect(text).toContain('npm publish "$RUNNER_TEMP/npm/xiopt-pane-dash-opencode-0.1.5.tgz" --access public --provenance')
...
test("npm production audits signatures without fallback bindings and verifies publication order", async () => {
  ...
  const plugin = npm.indexOf("xiopt-pane-dash-opencode-0.1.5.tgz")
  const cli = npm.indexOf("xiopt-tmux-pane-dash-0.1.5.tgz")
...
test("npm production audits only the exact published versions in an isolated no-auth project", async () => {
  const text = await workflow("release.yml")
  const npm = job(text, "npm-production")
  const install = 'npm_config_userconfig="$audit_dir/.npmrc" npm_config_cache="$RUNNER_TEMP/npm-audit-cache" npm install --prefix "$audit_dir" --ignore-scripts --no-audit --no-fund --package-lock=false @xiopt/pane-dash-opencode@0.1.5 @xiopt/tmux-pane-dash@0.1.5'
  ...
  const pluginPublish = npm.indexOf('npm publish "$RUNNER_TEMP/npm/xiopt-pane-dash-opencode-0.1.5.tgz"')
  const cliPublish = npm.indexOf('npm publish "$RUNNER_TEMP/npm/xiopt-tmux-pane-dash-0.1.5.tgz"')
...
test("draft validation verifies each exact release asset with the tagged workflow identity", async () => {
  ...
  expect(validation).toContain("--source-ref refs/tags/v0.1.5")
