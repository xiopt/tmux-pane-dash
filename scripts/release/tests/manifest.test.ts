...
import { TAG, TARGETS, VERSION } from "../contracts"
...
test("internal manifest has seven sorted payload records and exact schema", async () => {
  ...
  try {
    ...
    expect(await internalManifest({ target: TARGETS["darwin-arm64"].rustTarget, asset: TARGETS["darwin-arm64"].asset, root })).toMatchObject({ schemaVersion: 1, product: "tmux-pane-dash", version: VERSION, files: expect.any(Array) })
...
test("release manifest has four immutable exact keys and sorted two-space checksums", async () => {
  ...
  expect(Object.values(manifest.assets).every((asset) => asset.url === `https://github.com/xiopt/tmux-pane-dash/releases/download/${TAG}/${asset.asset}`)).toBe(true)
