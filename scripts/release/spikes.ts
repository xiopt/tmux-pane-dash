...
const opencodePluginManifest = JSON.parse(await readFile(join(import.meta.dir, "../../opencode-plugin/package.json"), "utf8")) as { version?: unknown }
const OPENCODE_PLUGIN_VERSION = opencodePluginManifest.version
if (typeof OPENCODE_PLUGIN_VERSION !== "string" || !/^\d+\.\d+\.\d+$/.test(OPENCODE_PLUGIN_VERSION)) throw new Error("OpenCode plugin package version must be semver")
const OPENCODE_PLUGIN_SPEC = `@xiopt/pane-dash-opencode@${OPENCODE_PLUGIN_VERSION}`
...
export async function observeOpenCodePluginSpec(value: string): Promise<{ readonly name: "@xiopt/pane-dash-opencode"; readonly rawSpec: string }> {
  ...
  try {
    ...
    if (parsed.version !== "13.0.2" || !modulePath || relative(packageRoot, modulePath).startsWith("..") || parsed.name !== "@xiopt/pane-dash-opencode" || parsed.rawSpec !== OPENCODE_PLUGIN_VERSION) throw new Error("exact scoped package observation required")
    return { name: parsed.name as "@xiopt/pane-dash-opencode", rawSpec: parsed.rawSpec as string }
  } finally {
...
export function assertOpenCodeRegistryRequests(requests: readonly string[], _origin: string): void {
  const expected = new Set([
    ...
    `/%40xiopt%2Fpane-dash-opencode/-/pane-dash-opencode-${OPENCODE_PLUGIN_VERSION}.tgz`,
  ])
