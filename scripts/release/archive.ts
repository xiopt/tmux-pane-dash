...
export async function inspectArchive(path: string): Promise<Array<[string, "file", string]>> {
  ...
  for (let offset = 0; offset + 512 <= tar.length;) { const header = tar.subarray(offset, offset + 512); if (header.every((byte) => byte === 0)) break; const nul = header.indexOf(0); const name = new TextDecoder().decode(header.subarray(0, nul < 0 ? 100 : nul)); const size = Number.parseInt(new TextDecoder().decode(header.subarray(124, 136)).replace(/\0.*$/, "").trim(), 8) || 0; const mode = (Number.parseInt(new TextDecoder().decode(header.subarray(100, 108)).replace(/\0.*$/, "").trim(), 8) || 0).toString(8).padStart(4, "0"); if (header[156] !== 53) entries.push([name, "file", mode]); offset += 512 + Math.ceil(size / 512) * 512 }
  const expected = new Map(ARCHIVE_PAYLOAD); if (entries.length !== expected.size || entries.some(([name, , mode]) => expected.get(name) !== mode)) throw new Error("archive inventory is not exact")
