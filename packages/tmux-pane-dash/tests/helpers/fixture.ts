...
export function internalManifest(version = "0.1.0") { return { schemaVersion: 1 as const, product: "tmux-pane-dash" as const, version, target: record.target, asset, files: [...payload].sort(([a], [b]) => Buffer.from(a).compare(Buffer.from(b))).map(([path, [content, mode]]) => ({ path, sha256: createHash("sha256").update(content).digest("hex"), size: Buffer.byteLength(content), mode: mode.toString(8).padStart(4, "0") as "0755" | "0644" })) } }
export function fixtureDependencies(input: { responses?: FetchResponse[]; fault?: string } = {}) {
  ...
  const counted: FsOps = {
    mkdir: async (path) => { calls.fs += 1; fail("fs.mkdir"); return fs.mkdir(path) }, readFile: async (path) => { calls.fs += 1; fail("fs.readFile"); return fs.readFile(path) }, writeFileExclusive: async (...args) => { calls.fs += 1; fail("fs.writeFileExclusive"); return fs.writeFileExclusive(...args) }, stat: async (path) => { calls.fs += 1; fail("fs.stat"); return fs.stat(path) }, readdir: async (path) => { calls.fs += 1; fail("fs.readdir"); return fs.readdir(path) }, rm: async (path) => { calls.fs += 1; return fs.rm(path) },
  }
  const deps: Dependencies = { manifest: {}, platform: "linux", arch: "x64", executingVersion: "0.1.0", fs: counted, nowMs: () => 0, fetch: async () => { calls.fetch += 1; fail("fetch"); return input.responses?.shift() ?? { status: 500 } }, spawn: async () => { calls.child += 1; fail("child"); return { code: 0, stdout: "0.1.0\n", stderr: "" } }, timers: { setTimeout: (callback) => { calls.timer += 1; fail("timer"); return callback }, clearTimeout: () => {} }, signals: { on: () => { calls.signal += 1; fail("signal") }, off: () => { calls.signal += 1 } } }
