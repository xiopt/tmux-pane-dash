...
async function fixtureManifest(version: "0.1.2" | "0.1.3", selected: ReleaseAssetRecord) {
...
test("lifecycle: packed runtime installs, updates, rolls back, and uninstalls on the matching host", async () => {
  ...
  try {
    ...
    const releases = await Promise.all((["0.1.2", "0.1.3"] as const).map(version => buildFixtureRelease({ version, target: rustTarget(), binary: "pane-dash", root })))
    ...
    try {
      const oldDeps = runtimeDependencies({ manifest: await fixtureManifest("0.1.2", releases[0]!), version: "0.1.2", fetch: server.fetch, env: h.env, output })
      ...
      await cli(runtime, ["setup"], oldDeps); expect(server.requestsFor("0.1.2")).toBe(1)
      expect(await fixedPathTmux()).toBe(tmuxBin)
      expect(h.env.TMUX_TMPDIR.startsWith(dirname(h.env.HOME))).toBe(true)
      const newDeps = runtimeDependencies({ manifest: await fixtureManifest("0.1.3", releases[1]!), version: "0.1.3", fetch: server.fetch, env: h.env, output })
      const oldPopup = await launchBoundPopup(managed, h.env, tmuxBin)
      try {
        expect(await oldPopup.version()).toBe("0.1.2")
...
`)
        ...
        expect(await oldPopup.alive()).toBe(true); expect(await oldPopup.version()).toBe("0.1.2")
        ...
        try { expect(await newPopup.alive()).toBe(true); expect(await newPopup.version()).toBe("0.1.3") } finally { await newPopup.cleanup() }
      ...
      expect(server.requestsFor("0.1.2")).toBe(conflictBefore.requests)
      ...
      expect(h.sentinel.requests).toEqual([]); expect(server.requestsFor("0.1.2") + server.requestsFor("0.1.3")).toBeGreaterThan(0)
