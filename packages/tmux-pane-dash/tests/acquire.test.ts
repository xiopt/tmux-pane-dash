...
import { archiveRecord, asset, fixtureDependencies, installedFixture, record, releaseArchive } from "./helpers/fixture"
...
test("reuses an exact healthy payload offline and stages a real archive outside payload", async () => {
  ...
  const root = await temp("stage-real"), { bytes, record: stagedRecord } = goodDownload(), h = fixtureDependencies({ responses: [response(bytes)] })
  try {
...
test("rejects absent fetch, unsupported platform, and record mismatch before fetch", async () => {
  ...
  await expect(downloadAsset(downloadRecord, join(tmpdir(), crypto.randomUUID()), h.deps)).rejects.toThrow("E_DOWNLOAD_FETCH")
})
...
test("cleans partial downloads for final response, body, size, hash, and exclusive collisions", async () => {
  ...
  for (const [name, recordChange, item] of [["status", {}, response(bytes, 500)], ["missing", {}, { status: 200 }], ["early", { size: bytes.length + 1 }, response(bytes)], ["extra", { size: bytes.length - 1 }, response(bytes)], ["cap", { size: 64 * 1024 * 1024 + 1 }, response(bytes)], ["hash", { sha256: "0".repeat(64) }, response(bytes)]] as const) {
    ...
    try { await expect(downloadAsset({ ...downloadRecord, ...recordChange }, destination, h.deps)).rejects.toThrow(); expect(await Bun.file(destination).exists()).toBeFalse() } finally { await rm(root, { recursive: true, force: true }) }
  }
  ...
  try { await expect(downloadAsset(downloadRecord, destination, h.deps)).rejects.toThrow(); expect(await Bun.file(destination).text()).toBe("keep") } finally { await rm(root, { recursive: true, force: true }) }
})
...
test("timer and signal abort races settle promptly and clear every active handle once", async () => {
  for (const trigger of ["response", "stall", "total", "HUP", "INT", "TERM"] as const) {
    ...
    const action = downloadAsset(downloadRecord, join(tmpdir(), crypto.randomUUID()), h.deps); await Promise.resolve()
