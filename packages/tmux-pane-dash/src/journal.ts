...
export type JournalMutation = { operation: "version" | "config" | "current" | "ownership" | "tombstone"; logicalPath: string; resolvedPath: string; pre: FileState; post: FileState; preimage: string | null; applied: boolean; backupPath?: string; symlinkChain?: readonly { path: string; target: string; dev: number; ino: number }[] }
...
function valid(value: any): value is Journal { return value && value.schemaVersion === 1 && typeof value.id === "string" && /^[a-f0-9-]{16,}$/.test(value.id) && ["setup", "update", "uninstall"].includes(value.command) && typeof value.packageVersion === "string" && journalPhases.includes(value.phase) && (value.previousCurrent === null || typeof value.previousCurrent === "string") && value.components && typeof value.components.tmux === "boolean" && typeof value.components.opencode === "boolean" && Array.isArray(value.mutations) && value.mutations.every((m: any) => m && ["version", "config", "current", "ownership", "tombstone"].includes(m.operation) && typeof m.logicalPath === "string" && typeof m.resolvedPath === "string" && validState(m.pre) && validState(m.post) && (m.preimage === null || typeof m.preimage === "string") && (m.backupPath === undefined || typeof m.backupPath === "string") && (m.symlinkChain === undefined || Array.isArray(m.symlinkChain) && m.symlinkChain.every((link: any) => link && typeof link.path === "string" && typeof link.target === "string" && Number.isInteger(link.dev) && Number.isInteger(link.ino))) && typeof m.applied === "boolean") }
...
export async function persistPreimage(root: string, id: string, path: string, bytes: Uint8Array, deps: Dependencies): Promise<void> { await durableWrite(join(root, "transactions", id, path), bytes, deps) }
